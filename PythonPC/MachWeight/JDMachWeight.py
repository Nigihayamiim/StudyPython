...
class CrawlInfo(Thread):
    ...
    def run(self):
        ...
        try:
            response = requests.post(Num_url, headers=headers, data=form_data)
        except Exception:
            print("第" + str(num) + "页数据出现超时,尝试再次连接")
            response = requests.post(self.url, headers=headers, data=form_data)
        if response.status_code == 200:
            e = etree.HTML(response.text)
            order_nums = e.xpath('//div[3]/div[2]/a')
            order_pers = e.xpath('//div[3]/dl/dd[3]/em')
            order_times = e.xpath('//div[3]/dl/dd[2]/em')

            for order_num, order_per, order_time in order_nums, order_pers, order_times:
                try:
...
if __name__ == '__main__':
    ...
    Num_url = "https://biz-wb.jdwl.com/business/waybillmanage/index?isFromTab=1"
